               ...
               #==========================================================Functions====================================================================
                     def StudentRec(event):
                            try: 
                                   ...
                                   self.Entry_admission.delete(0,END)
                                   self.Entry_admission.insert(END, selected_tuple[4])
                                   ...
                                   self.Entry_gender.insert(END, selected_tuple[9])
                            except IndexError:
                                   pass
                     ...
                     def Add():
                            if(len(self.name.get()) != 0):
                               Std_info_BackEnd.insert(END,(self.name.get(), self.fname.get(), self.mname.get(),self.val . admission.get(),self.address.get(), self.mobno.get(), self.email.get(), self.dob.get(), \
                                                       self.gender.get()))
                               self.listbox.delete(0, END)
                               self.listbox.insert(END, (self.name.get(), self.fname.get(), self.mname.get(),self.admission.get(), self.address.get(), self.mobno.get(), self.email.get(), self.dob.get(), \
                                                       self.gender.get()))
                     ...
                     def Exit():
                            Exit = tkinter.messagebox.askyesno("Login", "Confirm if you want to Exit")
                            if Exit > 0:
                                   self.master.destroy()
                                   return 
                     ...
                     def Search():
                            self.listbox.delete(0, END)
                            for row in Std_info_BackEnd.search(self.name.get(), self.fname.get(), self.mname.get(),self.admission.get(),self.address.get(), self.mobno.get(), self.email.get(), self.dob.get(),self.gender.get()):
                                   self.listbox.insert(END, row, str(' '))
                     ...
                     def Update():
                            if(len(self.name.get()) != 0):
                               Std_info_BackEnd.delete(selected_tuple[0])
                            if(len(self.name.get()) != 0):
                               Std_info_BackEnd.insert(self.name.get(), self.fname.get(), self.mname.get(),self.admission.get(), self.address.get(), self.mobno.get(), self.email.get(), self.dob.get(), \
                                                       self.gender.get())
                     ...
                     self.Frame_1 = LabelFrame(self.Main_Frame, width = 600, height = 400, font = ('arial',15,'bold'), \
                                               relief = 'ridge', bd = 10, bg = 'skyblue', text = 'STUDENT INFORMATION ')
                     self.Frame_1.grid(row = 1, column = 0, padx = 10)

                     self.Frame_2 = LabelFrame(self.Main_Frame, width = 750, height = 400, font = ('arial',15,'bold'), \
                                               relief = 'ridge', bd = 10, bg = 'skyblue', text = 'STUDENT DATABASE')
                     self.Frame_2.grid(row = 1, column = 1, padx = 5)                  
                     
                     self.Frame_3 = LabelFrame(self.master, width = 1300, height = 150, font = ('arial',10,'bold'), \
                                               bg = 'skyblue', relief = 'ridge', bd = 13)
                     self.Frame_3.grid(row = 2, column = 0, pady = 15)


                     
                     #========================================================Labels of Frame_1========================================================
                     self.Label_name = Label(self.Frame_1, text = 'Name', font = ('arial',20,'bold'),  bg = 'skyblue')
                     self.Label_name.grid(row = 0, column = 0, sticky = W, padx = 20, pady = 10)
                     self.Label_fname = Label(self.Frame_1, text = 'Father Name', font = ('arial',20,'bold'),  bg = 'skyblue')
                     self.Label_fname.grid(row = 1, column = 0, sticky = W, padx = 20)
                     self.Label_mname = Label(self.Frame_1, text = 'Mother Name', font = ('arial',20,'bold'),  bg = 'skyblue')
                     self.Label_mname.grid(row = 2, column = 0, sticky = W, padx = 20)
                     self.Label_admission = Label(self.Frame_1, text ='Admission No', font = ('arial',20,'bold'),  bg = 'skyblue')
                     self.Label_admission.grid(row = 3, column = 0, sticky = W, padx = 20)
                     self.Label_address = Label(self.Frame_1, text = 'Address', font = ('arial',20,'bold'),  bg = 'skyblue')
                     self.Label_address.grid(row = 4, column = 0, sticky = W, padx = 20)
                     self.Label_mobno = Label(self.Frame_1, text = 'Mobile Number', font = ('arial',20,'bold'),  bg = 'skyblue')
                     self.Label_mobno.grid(row = 5, column = 0, sticky = W, padx = 20)
                     self.Label_emailID = Label(self.Frame_1, text = 'Email ID', font = ('arial',20,'bold'),  bg = 'skyblue')
                     self.Label_emailID.grid(row = 6, column = 0, sticky = W, padx = 20)
                     self.Label_dob = Label(self.Frame_1, text = 'Date of Birth', font = ('arial',20,'bold'),  bg = 'skyblue')
                     self.Label_dob.grid(row = 7, column = 0, sticky = W, padx = 20)
                     self.Label_gender = Label(self.Frame_1, text = 'Gender', font = ('arial',20,'bold'),  bg = 'skyblue')
                     ...
                     self.Entry_admission = Entry(self.Frame_1, font = ('arial',17,'bold'), textvariable = self.admission)
                     self.Entry_admission.grid(row = 3, column = 1, padx = 10, pady = 5)
